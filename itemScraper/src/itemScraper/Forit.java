...
public class Forit extends Utils{
	public void foritResults(String cautare, JTable jtableG) {
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("input#search_instock")));
		...
		if(elementePrimaPag > 0) {
			while(true) {
				...
				wait.until(ExpectedConditions.refreshed(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//form[@class='addtocart-modal']//h5[@class='name']/a"))));
				for (int j = 0; j < count; j++) {
					String nume = driver.findElements(By.xpath("//form[@class='addtocart-modal']//h5[@class='name']/a")).get(j).getText().replaceAll(",", "");
